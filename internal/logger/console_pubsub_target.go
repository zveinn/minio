...
// ConsolePubSubTarget holds global console logger state
type ConsolePubSubTarget struct {
	nodeName             string
...
// NewConsolePubSubLogger - creates new HTTPConsoleLoggerSys with all nodes subscribed to
// the console logging pub sub system
func NewConsolePubSubLogger(
	ctx context.Context,
	isDistributedErasure bool,
	globalLocalNodeName string,
	writer io.Writer,
) *ConsolePubSubTarget {
	return &ConsolePubSubTarget{
		pubsub:               pubsub.New[log.Info, madmin.LogMask](8),
		logBuf:               ring.New(defaultLogBufferCount),
		Writer:               writer,
		isDistributedErasure: isDistributedErasure,
	}
}

// SetNodeName - sets the node name if any after distributed setup has initialized
func (sys *ConsolePubSubTarget) SetNodeName(nodeName string) {
...
func (sys *ConsolePubSubTarget) Subscribe(subCh chan log.Info, doneCh <-chan struct{}, node string, last int, logKind madmin.LogMask, filter func(entry log.Info) bool) error {
...
// log pubsub system
func (sys *ConsolePubSubTarget) Write(entry interface{}) error {
	var lg log.Info
	switch e := entry.(type) {
	case *log.Entry:
...
func (c *ConsolePubSubTarget) Send(e interface{}) error {
