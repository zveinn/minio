...
func (g *GlobalBatchQueue) NewTarget(ctx context.Context, t *QueueTarget) {
	g.Targets = append(g.Targets, t)
	go t.launchFanOutWorker(ctx, g.BatchManager, true)
	return
}
...
func (t *QueueTarget) spawnMoreWorkers(ctx context.Context, bm *DiskBatchManager) {
	if len(t.Ch) < cap(t.Ch)/3 {
		return
	}
	...
	if state == 0 && count < 5 && count < t.maxWorkers {
		...
		go t.launchFanOutWorker(ctx, bm, false)
...
func (t *QueueTarget) launchFanOutWorker(ctx context.Context, batchManager *DiskBatchManager, isPrimary bool) {
	atomic.AddInt64(&t.workerCount, 1)
	defer atomic.AddInt64(&t.workerCount, -1)

	var shouldRestart bool = true
	var currentBatchUUID string
	var currentBatchCount int
	var lastSuccessIndex int
	var lastSendFailed bool
	var batch *Batch
	var written int
	var failed int
	var err error
	...
	defer func() {
		...
		if isPrimary && shouldRestart {
			go t.launchFanOutWorker(ctx, batchManager, isPrimary)
		}
	}()
	...
	postWriteOps := func(err error) (shouldRetry bool) {
		if isPrimary && t.maxWorkers > 1 {
			t.spawnMoreWorkers(ctx, batchManager)
		}
	...
	for {
		...
		retry:
			...
			if batchManager != nil {
				batchManager.BatchStateCh <- &batchStateUpdate{
					Target:           t.name,
...
) (L *console.ConsoleByteTarget) {
	L = console.NewConsoleByteTarget(ctx, optionalReplacementWriter)
